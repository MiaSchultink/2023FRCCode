...
public class SwerveModule extends SubsystemBase {
    ...
    public void setDesiredState(SwerveModuleState desiredState) {
        ...
        double desiredAngle = (Math.abs(desiredState.speedMetersPerSecond) <= (Constants.Swerve.maxSpeed * 0.01))
                ? lastAngle
    ...
    public SwerveModuleState getState() { // state is funtion of velocity and angle
        ...
        Rotation2d angle = Rotation2d.fromDegrees(
                Conversions.falconToDegrees(turnMotor.getSelectedSensorPosition(), Constants.Swerve.angleGearRatio));
        SwerveModuleState state = new SwerveModuleState(velocity, angle);
        return state;
    }

    private void resetToAbsolute() {
        double absolutePosition = Conversions.degreesToFalcon(getCanCoderValue().getDegrees() - this.offsetAngle,
    ...
    public SwerveModulePosition getPosition() {
        return new SwerveModulePosition(
                Conversions.falconToMeters(driveMotor.getSelectedSensorPosition(), Constants.Swerve.wheelCircumference,
